...
def use_sort():
    ...
    print(
        "对列表进行排序，按照绝对值排序：", data2
    )  # Output: [1, 2, 3, 4, 5] - abs is not needed here as all are positive
...
def fun1():
    ...
    var4 = 400
    print(
        f"func1的局部变量 variables: var1:{var1}, var2:{var2}, var3:{var3}, var4:{var4}"
    )
...
def use_Closure():
    ...
    for _ in range(10):
        print(
            "第%s次调用闭包函数:" % (_ + 1), closure_func()
        )  # Output: 1, 2, 3 - each call increments the local variable in the closure


# 模块
import src.DataAnalysis.Proficient.test1 as test1
...
from src.DataAnalysis.Proficient.test1 import show, add
...
def bubble_sort(arr):
    ...
    for i in range(n):
        ...
        print(
            f"After pass {i+1}: {arr}"
        )  # Output: After pass 1: [34, 25, 12, 22, 11, 64, 90]
    return arr
...
def use_metaclass():
    ...
    print(
        "对象的类型:", type(chicken)
    )  # Output: 对象的类型: <class '__main__.Chicken'>
    ...
    class Chicken:
        ...
        def weight(self, value):
            ...
            if value < 0.1 or value > 12.0:
                raise ValueError(
                    "Weight cannot be negative(< 0.1) or greater than 12.0"
                )
...
def use_quick_assignment():
    ...
    print(
        f"person1: {person1}"
    )  # Output: Person(name=张三, age=25, height=175, weight=70)
    person2 = Person(ID=2, name="李四", age=30, height=180, weight=75)
    print(
        f"person2: {person2}"
    )  # Output: Person(name=李四, age=30, height=180, weight=75)
    ...
    print(
        f"person2 after quick assignment: {person2}"
    )  # Output: Person(name=张三, age=25, height=175, weight=70)
...
def use_process_pool():
    ...
    for i in range(5):
        ...
        pool.apply_async(
            worker, args=(process_num,), callback=lambda x: print(f"Callback: {x}")
        )
...
def use_Multi_process_Statistics():
    ...
    queue = Queue()
    producer = Process(
        target=generate_data, args=(queue, "example.log")
    )  # 假设data.txt文件中包含多行数据，每行数据可能包含'spark'
