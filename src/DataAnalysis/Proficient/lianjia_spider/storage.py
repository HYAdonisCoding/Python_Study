...
from src.DataAnalysis.Proficient.lianjia_spider.parser import BASE_DIR
...
class SpiderDB:
    ...
    def save_house_list(self, house_list):
        """批量保存列表页信息"""
        cur = self.conn.cursor()
        local_time = datetime.now(timezone(timedelta(hours=8))).isoformat(
            timespec="seconds"
        )
        values = []
        for h in house_list:
            ...
            unit_price = float(
                str(h["unit_price"]).replace("元/平", "").replace(",", "")
            )
    ...
    def save_house_detail(self, detail):
        ...
        """,
            (
                detail["house_code"],
                detail.get("house_type"),
                detail.get("floor_info"),
                build_area,
                inner_area,
                detail.get("structure_type"),
                detail.get("construction"),
                detail.get("staircase_ratio"),
                detail.get("heating_type"),
                has_elevator,
                detail.get("floor_height"),
                detail.get("listing_date"),
                detail.get("ownership_type"),
                detail.get("last_transaction"),
                detail.get("usage"),
                detail.get("property_age"),
                detail.get("property_right"),
                detail.get("mortgage_info"),
                detail.get("documents_uploaded"),
                tags_json,
                detail.get("core_selling_points"),
                detail.get("community_intro"),
                local_time,
            ),
        )
        self.conn.commit()
...
def test_db_operations():
    ...
    sample_travels = [
        {
            "house_id": "101133895297",
            "detail_url": "https://bj.lianjia.com/ershoufang/101133895297.html",
            "title": "金隅丽港06年小区 人车分流 满五唯一、精致一居室",
            "community_name": "金隅丽港城",
            "community_url": "https://bj.lianjia.com/xiaoqu/1111027377528/",
            "region": "望京",
            "region_url": "https://bj.lianjia.com/ershoufang/wangjing/",
            "bedrooms": "1室1厅",
            "area": "54.8平米",
            "orientation": "东",
            "renovation": "简装",
            ...
            "building_type": "塔楼",
            "followers": 11,
            "publish_time": "31天以前发布",
            "total_price": "255",
            "unit_price": "46,533元/平",
        }
    ]
    ...
    sample_detail = {
        "total_price": "255万",
        "unit_price": "46533元/平米",
        "house_code": "101133895297",
        "house_type": "1室1厅1厨1卫",
        "floor_info": "低楼层 (共26层)",
        "build_area": "54.8㎡",
        "structure_type": "平层",
        ...
        "construction": "钢混结构",
        "renovation": "简装",
        "staircase_ratio": "三梯十二户",
        "heating_type": "集中供暖",
        "has_elevator": "有",
        "floor_height": "暂无数据",
        "listing_date": "2025-10-24",
        "ownership_type": "商品房",
        "last_transaction": "2021-12-04",
        "usage": "普通住宅",
        "property_age": "满两年",
        "property_right": "非共有",
        "mortgage_info": "有抵押 110万元 光大银行 客户偿还",
        "documents_uploaded": "已上传房本照片",
        "tags": ["地铁", "VR房源", "房本满两年"],
        "core_selling_points": "小区总价低大1居，高原值\n税费低，要比满五唯一税费还要低！",
        "community_intro": "金隅丽港城2006年小区，人车分流\n小区4栋板楼、3栋塔楼，国企开发，户户带飘窗阳台。\n洋房成本低，物业费1.7、车位350/月地下固定！",
    }
