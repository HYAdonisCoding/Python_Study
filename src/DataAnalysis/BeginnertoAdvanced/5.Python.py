...
from src.DataAnalysis.BeginnertoAdvanced.base_class import speter, BASE_DIR
import pandas as pd
import numpy as np
...
def distribution_shape():
    ...
    # 绘制直方图
    plt.hist(x=forestfires.area, bins=50, color="steelblue", edgecolor="black")
    # 显示图形
    plt.show()
...
def correlation_analysis():
    ...
    # 绘制PE与AT之间的散点图
    plt.scatter(x=ccpp.AT, y=ccpp.PE, c="steelblue", alpha=0.7, edgecolors="black")
    plt.xlabel("AT")
    plt.ylabel("PE")
    plt.show()
...
def independent_samples_t_test():
    ...
    print(
        "独立样本t检验",
        stats.ttest_ind(a=male_tips, b=female_tips, equal_var=True),  # 指定样本方差相等
    )
...
def parameter_estimation_for_multiple_linear_regression_models():
    ...
    # 4️⃣ 再建模（注意列名已变）
    fit = sm.formula.ols("sales ~ tv + radio + newspaper", data=sales).fit()
    print("模型的参数值：\n", fit.params)
...
def no_newspaper_for_multiple_linear_regression_models():
    ...
    # 4️⃣ 再建模（注意列名已变）
    fit = sm.formula.ols("sales ~ tv + radio", data=sales).fit()
    print("模型的参数值：\n", fit.params)
    print("模型的概览：\n", fit.summary())
    """
...
def predictions_models():
    ...
    # 删除无意义的变量
    titanic.drop(
        labels=["PassengerId", "Name", "Ticket", "Cabin", "Embarked"],
        axis=1,
        inplace=True,
    )
    print(titanic.head())
    ...
    # 删除原始的分类变量
    titanic_new.drop(
        labels=["Pclass", "Sex", "Pclass_3", "Sex_male"], axis=1, inplace=True
    )
    print(titanic_new.head())
    ...
    lm_Class = sm.formula.ols("Age ~ " + " + ".join(predictors), data=no_missing)
    # 拟合多元线性回归模型
    lm = lm_Class.fit()
    print("多元线性回归模型的参数值:\n", lm.summary())
    ...
    lm_Class_new = sm.formula.ols(
        "Age ~ " + " + ".join(predictors_new), data=no_missing
    )
    # 拟合新的多元线性回归模型
